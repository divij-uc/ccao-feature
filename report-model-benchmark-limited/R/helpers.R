get_timings <- function(machine,
  ...
  run_id <- glue("{machine}-{model_type}-{model_version}-{device_type}-{technique}")
  ...
  test_data %>%
    ...
    arrow::write_parquet(glue("report-model-benchmark-limited/output/performance/{run_id}-test.parquet"))
  ...
  full_data %>%
    select(price = meta_sale_price) %>%
    mutate(pred = predict(wflow_full_fit, full_data)$.pred) %>%
    summarize(
      run_id = run_id,
      time = lubridate::now(),
      rmse = rmse_vec(price, pred),
    ...
    arrow::write_parquet(glue("report-model-benchmark-limited/output/performance/{run_id}-full.parquet"))
  ...
  assessment_data_prepped <- assessment_data_prepped %>%
    mutate(pred = predict(
  ...
  assessment_data_prepped %>%
    ...
    arrow::write_parquet(glue("report-model-benchmark-limited/output/pred/{run_id}.parquet"))
  tictoc::toc(log = TRUE)
  ...
  #   z(wflow_eng_fit, shap_slice, type = "contrib") %>%
  #     as_tibble(.name_repair = "unique") %>%
  ...
  bind_rows(tictoc::tic.log(format = FALSE)) %>%
    ...
    arrow::write_parquet(glue("report-model-benchmark-limited/output/timing/{run_id}.parquet"))
}
