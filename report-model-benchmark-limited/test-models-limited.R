...
walk(list.files("report-model-benchmark-limited/R/", "\\.R$", full.names = TRUE), source)

# Load the parameters file containing the benchmark settings
params <- read_yaml("report-model-benchmark-limited/params_new.yaml")
...
training_data_full <- read_parquet("report-model-benchmark-limited/input/training_data.parquet") %>%
...
assessment_data <- as_tibble(read_parquet("report-model-benchmark-limited/input/assessment_data.parquet"))
...
if (params$run_cpu) {
  get_timings(
    ...
    technique = "no_nbhd_lat_lon_prox",
